...
@property (weak, nonatomic, readwrite) NSArray* currentRequests;
- (void) stopped;
@end
...
- (RESTOperation*) operationToStart: (BOOL)start {
    ...
    if (_headers.count > 0 || _oauth != nil) {
        ...
        if (_pull)
        {
            param = source = [NSMutableDictionary dictionaryWithObjectsAndKeys:source, @"url", _headers, @"headers", nil];
        }
        else
        {
            param = target = [NSMutableDictionary dictionaryWithObjectsAndKeys:target, @"url", _headers, @"headers", nil];
        }
        if (_oauth) {
        ...
        if (!op.isSuccessful) {
            ...
            if (_taskID) {
                ...
                COUCHLOG(@"%@: task ID = '%@'", self, _taskID);
                  // so I don't go away while active; see [self release] in -stopped
